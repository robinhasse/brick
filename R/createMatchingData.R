...
createMatchingData <- function(path, config, overwrite = FALSE) {
  ...
  getRefVars <- function(refMaps) {
    vars <- lapply(refMaps, getElement, "refVar") %>%
      unlist() %>%
      .unique()
  }
  ...
  referencesRel <- refConfig[sapply(refConfig[["isRelative"]], isTRUE) , "reference"]
  ...
  invisible(m$addSet(
    ...
    records = refVarBasic,
    description = "mapping each reference variables to all reference variables summed to basic value of a share reference"
  ))
